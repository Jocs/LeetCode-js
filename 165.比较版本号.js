...
var compareVersion = function(version1, version2) {
  const version1s = version1.split('.').map(v => +v)
  const version2s = version2.split('.').map(v => +v)
  const len = Math.max(version1s.length, version2s.length)

  for (let i = 0; i < len; i++) {
    const v1 = version1s[i] || 0
    const v2 = version2s[i] || 0
    if (v1 > v2) {
      return 1
    } else if (v1 < v2) {
      return -1
